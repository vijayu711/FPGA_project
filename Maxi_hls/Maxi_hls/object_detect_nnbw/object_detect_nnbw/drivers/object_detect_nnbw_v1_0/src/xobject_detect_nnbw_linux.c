...
typedef struct {
    ...
    XObject_detect_nnbw_uio_map maps[ MAX_UIO_MAPS ];
} XObject_detect_nnbw_uio_info;

/***************** Variable Definitions **************************************/
static XObject_detect_nnbw_uio_info uio_info;
...
static int uio_info_read_name(XObject_detect_nnbw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XObject_detect_nnbw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XObject_detect_nnbw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XObject_detect_nnbw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XObject_detect_nnbw_Initialize(XObject_detect_nnbw *InstancePtr, const char* InstanceName) {
	XObject_detect_nnbw_uio_info *InfoPtr = &uio_info;
